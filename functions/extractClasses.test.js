...
test("extractClasses processes CSS files and extracts class names", async () => {
  ...
  expect(mockReadFile).toHaveBeenCalledWith(path.join(__dirname, "..", "css", srcDir, "file1.css"), "utf8");
  expect(mockReadFile).toHaveBeenCalledWith(path.join(__dirname, "..", "css", srcDir, "file2.css"), "utf8");
  expect(mockMkdir).toHaveBeenCalledWith(path.join(__dirname, "..", srcDir, "file1"), { recursive: true });
  expect(mockMkdir).toHaveBeenCalledWith(path.join(__dirname, "..", srcDir, "file2"), { recursive: true });
  expect(mockWriteFile).toHaveBeenCalledWith(path.join(__dirname, "..", srcDir, "file1", "class.json"), JSON.stringify(["class1", "class2", "class3"], null, 2));
  expect(mockWriteFile).toHaveBeenCalledWith(path.join(__dirname, "..", srcDir, "file2", "class.json"), JSON.stringify(["class4", "class5"], null, 2));
...
test("extractClasses throws error if no CSS files are found", async () => {
  ...
  await expect(extractClasses({ srcDir })).rejects.toThrow("No CSS files found in the specified directory");
});
...
test("extractClasses throws error if reading directory fails", async () => {
  ...
  await expect(extractClasses({ srcDir })).rejects.toThrow("Error extracting classes: Directory not found");
});
