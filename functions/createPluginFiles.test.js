...
test("createPluginFiles creates object.js and index.js for base type", async () => {
  ...
  expect(mockWriteFile).toHaveBeenCalledWith(indexJsPath, `import ${fileName} from './object.js';
...
test("createPluginFiles throws error if writeFile fails", async () => {
  ...
  await expect(createPluginFiles(type, componentDir, jsContent, fileName)).rejects.toThrow(mockError);
});
