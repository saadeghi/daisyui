...
<div class="component-preview not-prose my-4 max-w-4xl" id={titleStr} bind:this={wrapper}>
  ...
  <div class="grid">
    ...
    {#if showContent == "preview"}
      <div class="relative overflow-x-auto rounded-b-xl rounded-tr-xl bg-base-300">
        <div class="preview flex min-h-[6rem] min-w-[24rem] max-w-4xl flex-wrap items-center justify-center gap-2 overflow-x-hidden rounded-b-xl rounded-tr-xl border border-base-300 bg-base-200 bg-cover bg-top	p-4 {classes}" style={bg ? `background-image: url(${bg})` : `background-size: 5px 5px`} class:resize-x={responsive}>
          <slot />
        </div>
      </div>
    {/if}
