...
<div class="prose mb-10">
  ...
  <p>
    You can add your custom themes to <span class="badge badge-outline">tailwind.config.js</span> file in <span class="badge badge-outline">daisyui > themes</span> array. 
    In this page, you can pick required color values and see how the components will look like with them.
  </p>
  <div class="alert alert-info text-sm not-prose mb-2">
    <div>
  ...
  <p>
    You can also define optional colors to have more control on the color values (for example: the color of button when it's focused of the color of the text on a button)
  </p>
  <p>
    Check out <a class="link font-bold" href="/docs/colors/">colors page</a> to see all the color names you can use.
  </p>
</div>
</div>
</div>
<div class="alert alert-info text-sm not-prose">
  <div>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-info-content flex-shrink-0 w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
    <div><p>
    check out <a class="link font-bold" href="/docs/themes/">themes page</a> to see all the CSS variables you can use to customize the design desicion (example: border-radius, animations, etc.)
  </p>
</div>
</div>
</div>
...
<div class="flex flex-col xl:flex-row gap-4">
  ...
  <h2 class="font-bold text-xl px-2 pb-4">tailwind.config.js</h2>
{#if browser}
  <div class="mockup-code relative not-prose">
    <button class="btn btn-xs absolute top-2 right-2 normal-case" on:click={()=>resetColors()}>Reset</button>
...
<code>          <span data-tip="Pick →" class:tooltip={colors.filter((item) => requiredColorNames.includes(item.name))[0] == color} class="tooltip-open tooltip-accent tooltip-left align-middle"><span class="w-1 inline-block"></span><ColorPicker bind:value={color.value} on:set={() => generateColors(color.value)} /></span>"{color.name}": "<span class="px-1 bg-neutral-focus hover:outline hover:outline-neutral-content/20 focus:outline focus:outline-neutral-content rounded-sm" contenteditable="true" bind:innerHTML={color.value} on:input={() => generateColors(color.value)} on:keyup={() => generateColors(color.value)}>{color.value}</span>",
</code>{/each}<code>{`          },
        },
      ],
    },`}</code>
