...
<div
  ...
  ${$page.path == "/" ? (switchNavbarStyle ? "bg-base-100 text-base-content shadow-sm" : "text-primary-content") : switchNavbarStyle ? "bg-base-100 text-base-content shadow-sm" : "bg-base-100 text-base-content"}
  `}
>
  <nav class="navbar w-full">
    <div class="flex flex-1 gap-1 lg:gap-2">
      <label for="drawer" class={`btn btn-square btn-ghost drawer-button ${$page.path != "/" ? "lg:hidden" : ""}`}>
      ...
      <div class={`flex items-center gap-2 ${$page.path == "/" ? "" : "lg:hidden"}`}>
        <a href="/" aria-current="page" aria-label="Homepage" class={`flex-0 btn btn-ghost active nuxt-link-active px-2 ${switchNavbarStyle || $page.path != "/" ? "" : "hidden"}`}>
          <div class="font-title text-primary inline-flex text-lg transition-all duration-200 md:text-3xl">
            <span class={`lowercase ${switchNavbarStyle || $page.path != "/" ? "text-primary" : "text-primary-content"}`}>daisy</span>
            <span class={`uppercase ${switchNavbarStyle || $page.path != "/" ? "text-base-content" : "text-primary-content"}`}>UI</span>
          </div>
        </a>
        <a href="https://github.com/saadeghi/daisyui/blob/master/CHANGELOG.md" target="_blank" rel="nofollow" class={`link link-hover font-mono text-xs text-opacity-50 ${switchNavbarStyle || $page.path != "/" ? "" : "hidden"}`}>
    ...
    <div class="flex-0">
      <div class={`items-center flex-none hidden ${$page.path == "/" ? "lg:block" : ""}`}>
        <a href="/components/button" class="btn btn-ghost drawer-button normal-case">
