    ...
    <nav class="w-full navbar">
      <div class="flex flex-1 lg:gap-2">
        ...
        <label for="drawer" class={`btn btn-square btn-ghost drawer-button transition-all duration-200 ${$page.path != '/' ? 'lg:invisible lg:w-0' : ''}`}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="inline-block w-5 h-5 stroke-current md:w-6 md:h-6"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
        </label>

        <a href="/" aria-current="page" aria-label="Homepage" class="px-2 flex-0 btn btn-ghost active nuxt-link-active">
          <div class="inline-flex text-lg transition-all duration-200 md:text-3xl font-title text-primary">
