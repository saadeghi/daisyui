...
<div class="not-prose mb-10 overflow-x-auto">
  <div class="tabs flex-nowrap whitespace-nowrap">
    {#each tabs as { name, href }}
      <a {href} class={`tab tab-lifted tab-lg ${$page.path == href ? "tab-active" : ""}`}>
        {name}
      </a>
    {/each}

    <a target="_blank" rel="noopener" href="https://github.com/saadeghi/daisyui/tree/master/examples" class="tab tab-lifted tab-lg">
      <span class="link flex items-center text-xs">
    ...
    <span class="tab tab-lifted tab-lg flex-grow cursor-default" />
  </div>
</div>
