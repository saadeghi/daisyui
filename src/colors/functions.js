  ...
  convertToHsl: function (input) {
    ...
    if (typeof input === "object" && input !== null) {
      Object.entries(input).forEach(([rule, value]) => {
        ...
        // auto generate neutral colors
        if (!input.hasOwnProperty("neutral")) {
          resultObj["--n"] = 0 + " " + 0 + "%" + " " + 100 + "%"
        }

        if (!input.hasOwnProperty("neutral-200")) {
          const darkerHslArray = Color(input["neutral"]).darken(0.1).hsl().round().array()
          resultObj["--n2"] = darkerHslArray[0] + " " + darkerHslArray[1] + "%" + " " + darkerHslArray[2] + "%"
        }

        if (!input.hasOwnProperty("neutral-300")) {
          if (input.hasOwnProperty("neutral-200")) {
            const darkerHslArray = Color(input["neutral-200"]).darken(0.1).hsl().round().array()
            resultObj["--n3"] = darkerHslArray[0] + " " + darkerHslArray[1] + "%" + " " + darkerHslArray[2] + "%"
          } else {
            const darkerHslArray = Color(input["neutral"]).darken(0.1).darken(0.1).hsl().round().array()
            resultObj["--n3"] = darkerHslArray[0] + " " + darkerHslArray[1] + "%" + " " + darkerHslArray[2] + "%"
          }
        }
