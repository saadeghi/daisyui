<script>
  ...
  onMount(() => {
    ...
    const handleClick = async (event) => {
      ...
      const encodedCode = decodeURIComponent(button.dataset.copyCode || "")
      const resolvedCode = (renderedCode.length > 0 ? renderedCode : encodedCode).replaceAll(
        "$$",
        get(prefix),
      )

      await navigator.clipboard.writeText(resolvedCode)
      iconPath?.setAttribute("d", copiedIconPath)
      ...
      const timer = setTimeout(() => {
        iconPath?.setAttribute("d", copyIconPath)
