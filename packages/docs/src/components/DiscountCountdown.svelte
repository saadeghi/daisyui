<script>
  ...
  import { getDiscountCountdownRemaining } from "$lib/discountCountdown.js"
  import { discountDateFormat } from "$lib/storeDiscount.js"

  let { expiresAt, tooltip = false, variant = "error", class: className = "" } = $props()

  const datetime = $derived(new Date(expiresAt).toLocaleString("en-GB", discountDateFormat))
  let remaining = $state(getDiscountCountdownRemaining(null))

  onMount(() => {
    const timer = setInterval(() => {
      remaining = getDiscountCountdownRemaining(expiresAt)
