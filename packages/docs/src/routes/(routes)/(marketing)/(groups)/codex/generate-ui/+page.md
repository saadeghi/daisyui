...
[Codex](/codex/how-to-design-ui/) works inside a codebase, so its UI output has to match surrounding files. Without project-level component rules, it can generate valid markup that still feels unlike the rest of the app.

The fix is not a longer prompt for every task. The fix is a reusable [component vocabulary](/codex/ui-design-skill/) Codex can follow each time it edits frontend files.
