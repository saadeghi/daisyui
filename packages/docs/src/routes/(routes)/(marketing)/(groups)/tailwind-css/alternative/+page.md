...
Many developers looking for a [Tailwind CSS alternative](/tailwind-css/what-is-it-used-for/) do not dislike Tailwind. They dislike rebuilding common components with long utility lists. The problem is repetition, not the utility model itself.
...
This is still Tailwind CSS. daisyUI adds a [component layer](/tailwind-css/ui-components/) on top, so common UI does not need to be rebuilt from utilities on every page.
