...
<script>
  ...
  import MarketingPageLinks from "$components/MarketingPageLinks.svelte"
</script>
...
[Utility-first CSS](/tailwind-css/what-is-it-used-for/) solves naming and scoping problems, but it does not remove repeated UI work.
...
This is still Tailwind CSS. daisyUI adds a component layer on top, so [common UI](/tailwind-css/ui-components/) does not need to be rebuilt from utilities on every page.
