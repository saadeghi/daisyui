...
AI can produce [frontend code](/ai/frontend-code-generation/) quickly. The hard part starts after the first answer, when you ask for a smaller change and the model rewrites a wall of utility classes.

That happens because a [text prompt](/ai/prompt-to-ui/) leaves too many UI decisions open. A "nice dashboard" can mean cards, stats, tables, badges, filters, empty states, and several spacing systems. Without a shared component vocabulary, the model invents those details every time.
...
Use semantic components when you expect revisions, [multiple pages](/ai/design-website/), team handoff, or theme changes. For one experimental snippet, utility-only output can be fine. For a product surface, repeated utility chains turn into noise.
