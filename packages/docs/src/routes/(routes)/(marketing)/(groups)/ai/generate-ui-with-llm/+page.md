...
If the context contains long utility chains, the model has to reason through them. If the context contains [component names](/tailwind-css/ui-components/), it can operate at the level of the UI you meant.
...
That combination matters for [generated UI](/ai/llm-ui-generation/). Tailwind CSS remains available for layout and one-off styling, while daisyUI handles repeated interface parts with names a model can reuse: `btn`, `card`, `input`, `select`, `modal`, `navbar`, `menu`, `table`, `badge`, `alert`, `stat`, and `toast`.
