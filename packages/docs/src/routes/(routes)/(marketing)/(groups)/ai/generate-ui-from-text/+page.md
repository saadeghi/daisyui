...
The more abstract the [prompt](/ai/prompt-to-ui/), the more the model has to invent. Shared UI names turn a vague request into a set of concrete pieces.
...
Use semantic components when non-technical prompts need to become maintainable code. The prompt can stay [plain English](/ai/prompt-to-ui/), but the output should use a stable UI vocabulary.
