...
A UI design [MCP server for Claude Code](/claude-code/use-less-tokens/) is the better choice when static skill instructions are not enough. MCP lets Claude Code receive targeted daisyUI context during the task, instead of carrying every component rule inside the prompt.
