      ...
      <div class="sticky top-20 flex flex-col gap-6">
        <div class="flex flex-col gap-2">
          <div class="flex gap-2">
            {#if data.product.changelog?.length}
              <span
                class="tooltip"
                data-tip={`${data.product.changelog?.[0]?.changes?.length || 0} new changes`}
              >
                <button
              ...
              <dialog id="changelog" class="modal max-md:modal-bottom">
                ...
                >
                  <h3 class="font-title text-lg font-semibold">Changelog</h3>
                  <div class="py-4">
                    {#if data.product.changelog}
                      <div class="overflow-x-auto">
                        <table class="table">
                          ...
                          <tbody>
                            {#each data.product.changelog as version}
                              <tr>
                                <td class="align-top font-semibold whitespace-nowrap"
                                  >v{version.version}</td
                                >
                                <td>
                    ...
                    {:else}
                      <p class="text-base-content/60">No changelog available.</p>
                    {/if}
                  </div>
                </div>
