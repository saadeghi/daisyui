...
<SEO
  title="daisyUI MCP server for Antigravity"
  desc="Setup Antigravity to correctly generate daisyUI code based on your prompt."
/>
...
<h1><img src="https://img.daisyui.com/images/logos/antigravity.svg" alt="Antigravity" width="40" height="40" class="inline-block me-2 -mt-2 not-prose">daisyUI MCP server for Antigravity</h1>
<p>Setup Antigravity to correctly generate daisyUI code based on your prompt.</p>

MCP is a an API to communicate with AI models. You can add MCP servers and Antigravity will communicate with them to get more accurate results.  
...
2. Open the Antigravity MCP file at `~/.gemini/config/mcp_config.json`
3. Add this and set your `license key` + `email address` in it
  Figma API Key is optional - Only needed for Figma-to-code conversion

```diff:~/.gemini/config/mcp_config.json
{
  "mcpServers": {
+   "daisyui-blueprint": {
+     "type": "stdio",
...
#### Setup Context7 MCP server for Antigravity

1.  Open the Antigravity MCP file at `~/.gemini/config/mcp_config.json`
...
Now you can ask AI anything about daisyUI. write `use context7` at the end of your prompt.  
 For example:
...
#### Setup daisyUI GitMCP server for Antigravity
