...
<script>
  ...
  import SelectCustomContent from "$components/SelectCustomContent.svelte"
</script>
...
### ~Select with custom selected content display
#### Use `button` and `selectedcontent` elements to customize the look of the selected option. Make sure to use this as a progressive enhancement for browsers not supporting this feature yet.

<SelectCustomContent />

```html
<select class="$$select pe-2 bg-none group">
  <button>
    <selectedcontent></selectedcontent>
    <svg class="size-4 transition-transform group-open:rotate-180" viewBox="0 0 24 24" aria-hidden="true">
      <path
        d="M6 9l6 6 6-6"
        fill="none"
        stroke="currentColor"
  ...
  <option value="apple"><span class="text-lg">🍎</span> Apple</option>
  <option value="banana"><span class="text-lg">🍌</span> Banana</option>
  <option value="cherry"><span class="text-lg">🍒</span> Cherry</option>
</select>
```
